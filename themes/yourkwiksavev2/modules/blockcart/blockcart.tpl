...
<div id="cart_block_inner">
	...
	<div class="cart-prices">
		...
		{/if}
		<p id="cart-buttons">
			{if $order_process == 'order'}<a href="{$link->getPageLink("$order_process.php", true)}" class="exclusive_small" title="{l s='Cart' mod='blockcart'}">{l s='Cart' mod='blockcart'}</a>{/if}	
		
			<a href="{$link->getPageLink("$order_process.php", true)}{if $order_process == 'order'}?step=1{/if}" class="exclusive" id="button_order_cart" title="{l s='Check out' mod='blockcart'}">{l s='Check out' mod='blockcart'}</a>
