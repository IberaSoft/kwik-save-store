...
class BlockAdvertising2 extends Module
{
	...
	public function __construct()
	{
		...
		if (!file_exists(_PS_MODULE_DIR_.$this->name.'/'.$this->adv_imgname.'.'.Configuration::get('BLOCKADVERT_IMG_EXT2')))
		{
			$this->adv_imgname = 'advertising';
			if (!file_exists(_PS_MODULE_DIR_.$this->name.'/advertising.jpg'))
				$this->adv_imgname = '';
			else
				Configuration::updateValue('BLOCKADVERT_IMG_EXT2','jpg');
		}
		...
		if (Tools::isSubmit('submitAdvConf'))
		{
			...
			if (isset($_FILES['adv_img']) AND isset($_FILES['adv_img']['tmp_name']) AND !empty($_FILES['adv_img']['tmp_name']))
			{
				...
				elseif ($dot_pos = strrpos($_FILES['adv_img']['name'],'.'))
				{
					...
					Configuration::updateValue('BLOCKADVERT_IMG_EXT2',$ext);
					$this->adv_img = Tools::getMediaServer($this->name)._MODULE_DIR_.$this->name.'/'.$this->adv_imgname.'.'.Configuration::get('BLOCKADVERT_IMG_EXT2');
				}
			}
